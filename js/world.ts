...
import {applyPageModifiers} from './pages';
...
export function generateWorld(ascensionLevel = 0, equippedPages: Record<string, number> = {}) {
  ...
  for (let y = 0; y < MAP_H; y++) {
    for (let x = 0; x < MAP_W; x++) {
      ...
      const total = candidates.reduce((s, m) => s + (m.rarity ?? 0), 0);
...
export function worldToTile(px: number, py: number) {
  return { tx: Math.floor(px / TILE), ty: Math.floor(py / TILE) };
}

export function isSolidAt(px: number, py: number) {
